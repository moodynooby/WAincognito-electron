...
class WhatsAppElectron {
				...
				submenu: [
					...
					{ type: "separator" },
					{
						...
						click: () => {
							...
							if (views.length > 0) views[views.length - 1].webContents.openDevTools();
		...
		ipcMain.on(Constants.event.deleteAccount, (event, id) => {
			...
			for (let idx = 0; idx < this.menuTemplate[0].submenu.length; idx++) {
				if (this.menuTemplate[0].submenu[idx].type == "radio") {
					if (idx - 2 < 10) this.menuTemplate[0].submenu[idx].accelerator = `Alt+${idx - 1}`;

					if (idx - 2 == 10) this.menuTemplate[0].submenu[idx].accelerator = `Alt+0`;

					if (idx - 2 > 10) delete this.menuTemplate[0].submenu[idx].accelerator;
				}
			}
			this.menu = Menu.buildFromTemplate(this.menuTemplate);
			Menu.setApplicationMenu(this.menu);
	...
	createView(id, name) {
		...
		if (this.menuTemplate[0].submenu.length == 1) this.menuTemplate[0].submenu.push({ type: "separator" });
